...
class AppUserManager(BaseUserManager):
    ...
    def _create_user_object(self, email, password, **extra_fields):
        if not email:
            raise ValueError("Липсва адрес на електронна поща.")
    ...
    def with_perm(
        ...
        if backend is None:
            ...
            if len(backends) == 1:
                ...
                raise ValueError(
                    "Имате конфигурирани множество бекендове за удостоверяване и следователно трябва да предоставите аргумента `backend`."
                )
        elif not isinstance(backend, str):
            raise TypeError(
                "Бекендът трябва да бъде низ с път за импортиране с точки (got %r)." % backend
            )
        else:
